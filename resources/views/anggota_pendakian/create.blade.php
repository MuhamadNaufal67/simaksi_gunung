...
<div class="container">
    <h1>Tambah Anggota Pendakian</h1>
...
<form action="{{ route('anggota_pendakian.store') }}" method="POST">
    @csrf
    ...
    <div class="mb-3">
        ...
        <select name="id_pendaftaran" class="form-control" required>
            ...
            @foreach($pendaftarans as $p)
                <option value="{{ $p->id_pendaftaran }}">#{{ $p->id_pendaftaran }} - {{ $p->user->nama_lengkap ?? 'User' }}</option>
            @endforeach
        </select>
    </div>
